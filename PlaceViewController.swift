...
class PlaceViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    var tmpfavoriteID = UserDefaults.standard.array(forKey: "placeFavoriteid") as! [String]
    ...
    override func viewDidLoad() {
        ...
        if self.revealViewController() != nil {
            placeButton.target = self.revealViewController()
            placeButton.action = #selector(SWRevealViewController.revealToggle(_:))
            self.view.addGestureRecognizer(self.revealViewController().panGestureRecognizer())
        }
        self.tbView.tableFooterView = UIView(frame: CGRect.zero)
        print(text)
        self.preButton.isEnabled = false
        Alamofire.request("http://sample-env.xyhsmeh2ph.us-west-2.elasticbeanstalk.com/", parameters: ["search": text, "type": "place", "center": "0,0"]).validate().responseJSON {
            response in
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let star = cell.viewWithTag(302) as? UIImageView {
            if flag == true {
    ...
    override func viewWillAppear(_ animated: Bool) {
        tmpfavoriteID = UserDefaults.standard.array(forKey: "placeFavoriteid") as! [String]
        tbView.reloadData()
    }
