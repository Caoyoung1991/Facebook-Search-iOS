...
class EventViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
    var tmpfavoriteID = UserDefaults.standard.array(forKey: "eventFavoriteid") as! [String]
    ...
    @IBOutlet weak var preBUutton: UIButton!
    ...
    override func viewDidLoad() {
        ...
        if self.revealViewController() != nil {
            eventButton.target = self.revealViewController()
            eventButton.action = #selector(SWRevealViewController.revealToggle(_:))
            self.view.addGestureRecognizer(self.revealViewController().panGestureRecognizer())
        }
        self.tbView.tableFooterView = UIView(frame: CGRect.zero)
        print(text)
        self.preBUutton.isEnabled = false
        Alamofire.request("http://sample-env.xyhsmeh2ph.us-west-2.elasticbeanstalk.com/", parameters: ["search": text, "type": "event"]).validate().responseJSON {
            response in
    ...
    @IBAction func nextPage(_ sender: Any) {
        ...
        if self.nextPage != "" {
            ...
            imgURLArray = []
            idArray = []
            self.preBUutton.isEnabled = true
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let nameLabel = cell.viewWithTag(500) as? UILabel{
            nameLabel.text = name
        }
        ...
        if let star = cell.viewWithTag(502) as? UIImageView {
            if flag == true {
    ...
    override func viewWillAppear(_ animated: Bool) {
        tmpfavoriteID = UserDefaults.standard.array(forKey: "eventFavoriteid") as! [String]
        tbView.reloadData()
    }
