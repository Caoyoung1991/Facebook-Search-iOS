...
class GroupFavViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
    @IBOutlet weak var groupfavButton: UIBarButtonItem!

    @IBOutlet weak var tbView: UITableView!
    
    var idArray = UserDefaults.standard.array(forKey: "groupFavoriteid") as! [String]
    var nameArray = UserDefaults.standard.array(forKey: "groupFavoritename") as! [String]
    var imgURLArray = UserDefaults.standard.array(forKey: "groupFavoriteurl") as! [String]
    
    override func viewDidLoad() {
        super.viewDidLoad()
        SwiftSpinner.show("loading data...")
        if self.revealViewController() != nil {
            groupfavButton.target = self.revealViewController()
            groupfavButton.action = #selector(SWRevealViewController.revealToggle(_:))
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let nameLabel = cell.viewWithTag(4000) as? UILabel{
            nameLabel.text = name
        }
        let imgurl = NSURL(string: url)
        let imageView = cell.viewWithTag(4001) as! UIImageView
